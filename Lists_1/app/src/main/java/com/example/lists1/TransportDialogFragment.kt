...
class TransportDialogFragment : DialogFragment(R.layout.fragment_transport_dialog) {
    ...
    private val transportClickListener:TransportClickListener?
        get() = parentFragment?.let { it as ListFragment }
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        buttonAccept.setOnClickListener {
            when(transportType){
                0->{val airCraft = Transport.AirCraft(modelEditText.text.toString(),pictures[(0..2).random()],flightRangeEditText.text.toString())
                transportClickListener?.onTransportSelected(airCraft)
                }
                1->{val watercraft = Transport.WaterCraft(modelEditText.text.toString(),pictures[(3..5).random()],watercraftEditText.text.toString())
                    transportClickListener?.onTransportSelected(watercraft)}
                2->{val roadTransport = Transport.RoadTransport(modelEditText.text.toString(),pictures[(6..8).random()],fuelConsumptionEditText.text.toString())
...
companion object{
    ...
    fun newInstance(): TransportDialogFragment{
        val args = Bundle()
        val fragment = TransportDialogFragment()
